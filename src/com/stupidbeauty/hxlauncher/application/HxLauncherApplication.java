...
//       this.voicePackageUrlMap=voicePackageUrlMap;
  ...
  */
  ...
  /**
  * 寻找映射文件。包名与安装包路径之间的映射。陈欣。
  */
  private File findApkFilePathMapFile() 
  {
    File result = null;

    // ✅ 改用 getCacheDir() 替代 getFilesDir()
    ...
    {
      // 构造映射文件路径
      result = new File(cacheDir.getAbsolutePath() + "/apkFilePathMap.cx.d");
...
// 	private VoicePackageUrlMapData voicePackageUrlMapData; //!<语音识别结果与软件包下载地址之间的映射。
    ...
    // 使用第三方库提供的方法判断本地数据库是否存在
    if (!UpgradeManager.isLocalDatabaseExists(mContext)) 
    {
        loadVoicePackageUrlMap(); // 只有在没有本地数据库时才加载内置数据库
    }
