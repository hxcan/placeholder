...
//            for (FileMessageContainer.FileMessage currentSubFile:vfsFileMessage.getSubFilesList()) //一个个子文件地处理。
    ...
    private CBORObject loadVfsFile()
    {
        ...
        Log.w(TAG, "loadVfsFile, 468, timestamp: " + System.currentTimeMillis()); //Debug.
        ByteArrayOutputStream outputStream=new ByteArrayOutputStream();

        byte buf[]=new byte[65536];
        int len;
        Log.w(TAG, "loadVfsFile, 473, timestamp: " + System.currentTimeMillis()); //Debug.

        try
        {
            while ((len=ins.read(buf)) != -1)
            {
                Log.w(TAG, "loadVfsFile, 479, timestamp: " + System.currentTimeMillis()); //Debug.
                outputStream.write(buf,0,len);
            }

            outputStream.close();
            Log.w(TAG, "loadVfsFile, 484, timestamp: " + System.currentTimeMillis()); //Debug.
            ins.close();
        }
        catch (IOException e)
        {
            Log.w(TAG, "loadVfsFile, 489, timestamp: " + System.currentTimeMillis()); //Debug.
            e.printStackTrace();
        }

        //解析消息：
        Log.w(TAG, "loadVfsFile, 494, timestamp: " + System.currentTimeMillis()); //Debug.
